        ...
        <table class="table">
          ...
          <?php 
            ...
            if (isset($_GET['cari'])) {
                $nomor=1;
                $cari=$_GET['cari'];
                $sql=mysqli_query($host,"SELECT * FROM tanggapan INNER JOIN pengaduan ON tanggapan.id_pengaduan=pengaduan.id_pengaduan INNER JOIN masyarakat ON pengaduan.nik=masyarakat.nik INNER JOIN petugas ON tanggapan.id_petugas=petugas.id_petugas WHERE nama LIKE'%$cari%' or nama_user LIKE'%$cari%' or isi_laporan LIKE'%$cari%' or tanggapan LIKE'%$cari%' and pengaduan.nik='$_SESSION[nik]'");
              }else{
                $batas=4;
                $query=mysqli_query($host,"SELECT * FROM tanggapan INNER JOIN pengaduan ON tanggapan.id_pengaduan=pengaduan.id_pengaduan INNER JOIN masyarakat ON pengaduan.nik=masyarakat.nik WHERE pengaduan.nik='$_SESSION[nik]'");
                $jumlahdata=mysqli_num_rows($query);
                $jumlahHalaman= ceil($jumlahdata/$batas);
                $halamanAktif=(isset($_GET["halaman"]))?$_GET["halaman"]:1;
                $awaldata=($batas*$halamanAktif)-$batas;
                $nomor=1+$awaldata;
                $sql=mysqli_query($host,"SELECT * FROM tanggapan INNER JOIN pengaduan ON tanggapan.id_pengaduan=pengaduan.id_pengaduan INNER JOIN masyarakat ON pengaduan.nik=masyarakat.nik INNER JOIN petugas ON tanggapan.id_petugas=petugas.id_petugas WHERE pengaduan.nik='$_SESSION[nik]'  LIMIT $awaldata,$batas");
              }
            ...
            if ($cek>0) {
              ...
              ?>
              <tr>
                <td><h1 class ="my-1">Belum ada Tanggapan!</h1></td>
              </tr>
              <?php
            }
            ?>
