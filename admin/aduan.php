          ...
          <table class="table">
            ...
            <?php 
              ...
              if (isset($_GET['cari'])) {
                $cari=$_GET['cari'];
                $nomor=1;
                $sql=mysqli_query($host,"SELECT * FROM pengaduan INNER JOIN masyarakat ON pengaduan.nik=masyarakat.nik WHERE nama LIKE'%$cari%' or tgl_pengaduan LIKE'%$cari%' or isi_laporan LIKE'%$cari%' ORDER BY id_pengaduan DESC ");
              }else {
                ...
                $nomor=1+$awaldata;
                $sql=mysqli_query($host,"SELECT * FROM pengaduan INNER JOIN masyarakat ON pengaduan.nik=masyarakat.nik ORDER BY id_pengaduan DESC LIMIT $awaldata, $batas");
              }
                ...
                while ($dataaduan=mysqli_fetch_array($sql)) {
                  ...
                  <tr>
                    ...
                    <?php
                        if ($dataaduan['status']=="0") {
                          ?>
                          <a href="?url=detailaduan&id=<?php echo $dataaduan['id_pengaduan']?>" class="btn-sm btn btn-secondary">Detail & Proses</a>
                          <?php
                        }elseif ($dataaduan['status']=="Proses") {
                          ?>
                          <a href="?url=detailaduan&id=<?php echo $dataaduan['id_pengaduan']?>" class="btn-sm btn btn-secondary mr-1">Detail</a>
                          <?php
                            if ($_SESSION['level']=="Petugas") {
                            ?>
                          ...
                          ?>
                            <a href="?url=detailaduan&id=<?php echo $dataaduan['id_pengaduan']?>" class="btn-sm btn btn-secondary">Detail</a>
                          <?php
                        }else {
                          ?>
                          <a href="?url=detailaduan&id=<?php echo $dataaduan['id_pengaduan']?>" class="btn-sm btn btn-secondary mr-1">Detail</a>
                          <a href="?url=detailtanggapan&id_tanggapan=<?php echo $dataaduan['id_pengaduan']?>" class="btn-sm btn btn-primary">Detail Tanggapan</a>
                          <?php
                        }
                        ?>
